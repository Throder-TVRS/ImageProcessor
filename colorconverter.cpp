...
void ColorConverter::change_mixing_proportion(MIXER_TYPE mixer_type, int mixer_value) {
    switch(mixer_type) {
        case RED:
            _red_percentage = mixer_value;
            break;
        case GREEN:
            _green_percentage = mixer_value;
            break;
        case BLUE:
            _blue_percentage = mixer_value;
            break;
    }
}
